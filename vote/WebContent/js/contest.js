...
//		var index = layer.msg('查询中，请稍候',{icon: 16,time:false,shade:0.8});
		if($(".search_input").val() != ''){
			...
			window.location.href="selectContestByKey?key="+$(".search_input").val();
		}else{
	...
	form.on('switch(isShow)', function(data){
		var index = layer.msg('比赛开启中,请稍候',{icon: 16,time:false,shade:0.8});
		var _this = $(this);
		//alert(_this.attr("data-id"));
		layer.msg("展示状态修改成功！");
		window.location.href="updateContestStateById?battle_id="+_this.attr("data-id");
	...
	$("body").on("click",".news_edit",function(){  //编辑
		...
		var index = layui.layer.open({
			...
			content : "updateContestController?contest_id="+_this.attr("data-id"),
			success : function(layero, index){
				layui.layer.tips('点击此处返回用户列表', '.layui-layer-setwin .layui-layer-close', {
					tips: 3
				});
	...
	$("body").on("click",".news_del",function(){  //删除
		...
		//alert(_this.attr("data-id"));
			window.location.href="deleteContestById?battle_id="+_this.attr("data-id");
			layer.msg("删除成功");
			layer.close(index);
		});
	})
